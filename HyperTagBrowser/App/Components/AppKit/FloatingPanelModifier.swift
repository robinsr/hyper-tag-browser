...
private struct FloatingPanelModifier<PanelContent: View>: ViewModifier {
  ...
  @ViewBuilder let view: (NSPanel) -> PanelContent
  ...
  func body(content: Content) -> some View {
    content
      .onAppear {
        /// When the view appears, create, center and present the panel if Float
      ...
      .onChange(of: isPresented) { value, oldValue in
...
extension View {
  ...
  func floatingPanel<Content: View>(
    ...
    @ViewBuilder content: @escaping (NSPanel) -> Content
  ) -> some View {
