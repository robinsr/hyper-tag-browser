...
enum ModelActions: CustomStringConvertible, Equatable, Sendable {
  ...
  case applyIndexPatch(IndexRecord.Update)
  ...
  case copyDataToClipboard(label: String = "", value: Data)
  case ejectVolume(at: URL)
  
  // MARK: No-op Action
  
  /// Used in cases where a `ModelAction` type is expected, but no action should be taken
  case noop
  ...
  var description: String {
    switch self {
      ...
      case .applyIndexPatch(let patch):
        return "[updateIndex]: patch=\(patch)"
      ...
      case .copyDataToClipboard(let label, let data):
        return "[copyToClipboard]: label=[\(label)] value=[\(JSONEncoder.pretty(data))]"
      case .ejectVolume(at: let url):
        return "[ejectVolume]: at=[\(url.string)]"
  ...
  /**
   * Actions that should be executed with `SwiftUI.withAnimation`
   */
  ...
  var id: String {
    switch self {
      ...
      case .applyIndexPatch: "applyIndexPatch"
      ...
      case .copyDataToClipboard: "copyDataToClipboard"
