...
final class DetailScreenViewModel {
  ...
  @ObservationIgnored private let logger = EnvContainer.shared.logger("DetailScreenViewModel")
  @ObservationIgnored private let thumbnailStore = ThumbnailContainer.shared.store()
  @ObservationIgnored private let userPrefs = PreferencesContainer.shared.prefs()
  ...
  public func teardown() {
    ...
    self.fittedImage = .empty
    self.thumbnail = .empty
    self.imagePrimarColors = .defaults
    self.resetZoom()
  }
...
//        await self.getImagePrimarColors()
//      }.value
//      
//      let fittedImg = await Task.detached(priority: .userInitiated) {
//        await self.getContentImage(optimizeToViewSize: true)
//      }.value
      
//      let colors = await self.getImagePrimarColors()
      let fittedImg = await self.getContentImage()
      
      await MainActor.run {
//        self.imagePrimarColors = colors
        self.fittedImage = fittedImg
      }
