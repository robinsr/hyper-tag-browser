...
struct BrowseScreenToolbar: View {
  ...
  var ToggleListModeButton: some View {
    Toggle(isOn: .constant(parameters.mode.type == .recursive)) {
      Button("Include subfolder contents", .subcontents) {
        dispatch(.setListMode(parameters.mode.toggle))
      }
      .buttonStyle(.plain)
    }
  }
  ...
  var DebugTestAlertButton: some View {
    Button("Test Alert Toast", .error) {
      ...
      dispatch(.notify(
        AppMessage(TestLorem.sentences(count: 2), levels.randomElement()!)
      ))
    }
  }
