...
struct BrowseRefinements: View {
  ...
  var Filters_Items: some View {
    ForEach(currentFilters, id: \.id) { filter in
      TagButton(
        for: filter.tag,
        config: activeFilterTagButton(filter: filter)
      )
  ...
  var Suggestions_Items: some View {
    TagSuggestions(
      ...
      content: { i, countedTag in
        TagButton(for: countedTag) {
          searchResultTagButton(countedTag, at: i)
        }
      }
    )
  }
  ...
  func searchResultTagButton(_ record: CountedTagRecord, at index: Int) -> TagButtonConfiguration {
    let tapAction: ModelActions = .addFilter(record.asFilter, .inclusive)
    let longPressAction: ModelActions = .showSheet(.renameTagSheet(tag: record.asFilter, scope: .all))
    
    return TagButtonConfiguration(
      variant: .secondary,
      counts: .preference,
      menu: .tagMenu(when: .addingBrowseFilters),
      onMenuItem: dispatch,
      shortcut: .indexed(index, [.control]),
      onLongPress: { _ in
   ...
   */
  func activeFilterTagButton(filter: FilteringTag.Filter) -> TagButtonConfiguration {
    
    let invertAction: ModelActions = .invertFilter(filter.tag)
    let renameAction: ModelActions = .showSheet(.renameTagSheet(tag: filter.tag, scope: .visible))
    let changeDateAction: ModelActions = .showSheet(.datePickerSheet(tag: filter.tag))
    let removeAction: ModelActions = .removeFilter(filter.tag)
    
    let longPressAction: ModelActions = FirstTrueBuilder.withDefault(invertAction) {
      (filter.tag.domain == .creation, changeDateAction)
      (filter.tag.domain == .descriptive, renameAction)
    }
    
    let tapAction: ModelActions = FirstTrueBuilder.withDefault(removeAction) {
      (filter.tag.domain == .creation, changeDateAction)
    }

    return TagButtonConfiguration(
      variant: .primary(filter.effect),
      menu: .tagMenu(when: .editingBrowseFilters),
      onMenuItem: dispatch,
      onLongPress: { _ in
        dispatch(longPressAction)
      },
...
#Preview("Narrow", traits: .dbCtx, .app, .testBordersOff, .size(.tall3xl)) {
  VStack {
    BrowseRefinements()
  }
...
#Preview("Wide", traits: .dbCtx, .app, .testBordersOff, .size(.wide4xl)) {
  VStack {
    BrowseRefinements()
  }
  .preferredColorScheme(.dark)
