...
struct ErrorCallouts: View {
  ...
  @Environment(\.dbContentItemCount) var resultCount
  ...
  var itemLimit: Int {
    userPrefs.forKey(.photoGridItemLimit)
  }
  ...
  var ContentItemsTruncated: some View {
    ErrorScenarioCallout(
      ...
      help: "Showing up \("items", qty: itemLimit). Try adjusting your filters or choose a different sort option",
      icon: .eyeslash
    )
  }
  ...
  var WorkingDirVolumeNotMounted: some View {
    ErrorScenarioCallout(
      ...
      isTrue: {
        location.isMounted == false
      },
      title: "Volume not mounted",
      help: "Volume \(location.volumeName) is not mounted",
      icon: .noFolder
    )
  }
