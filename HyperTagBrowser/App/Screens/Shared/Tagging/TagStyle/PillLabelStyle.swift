...
struct PillLabelStyle: LabelStyle {
  ...
  let tagconfig: TagButtonConfiguration
  let isPressed: Binding<Bool>
  
  init(
    size: PillButtonSize,
    variant: PillButtonVariant = .primary,
    tagconfig: TagButtonConfiguration = .default,
  ...
  var showAsPressed: Bool {
    isPressed.wrappedValue || isActiveTag
  }
  ...
  var foregroundColor: Color {
    tagconfig.style.foreground[colorScheme].opacity(disabled ? 0.999 : 1.0)
  }

  var backgroundColor: Color {
    tagconfig.style.background[colorScheme].opacity(disabled ? 0.6 : 1.0)
  }

  var borderColor: Color {
    tagconfig.style.borderColor[colorScheme].opacity(disabled ? 0.999 : 1.0)
  }

  var borderWidth: Double {
    tagconfig.style.borderWidth[colorScheme]
  }
...
extension LabelStyle where Self == PillLabelStyle {
  ...
  static var smallpill: Self {
    PillLabelStyle(size: .small)
  }

  /// Applies the `.large` variant of ``PillLabelStyle`` to a `Label`
  static var largepill: Self {
    PillLabelStyle(size: .large)
  }
}
