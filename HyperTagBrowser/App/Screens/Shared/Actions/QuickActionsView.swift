  ...
  var items: [KBar.Item] {
    ...
    kbaritems.append(contentsOf: [
      ...
      KBar.Item(
        ...
        callback: { dispatch(.setListMode(.recursive(.cached))) }
      ),
      KBar.Item(
        title: "Folder Items",
        subtitle: "show contents of the current folder, excluding subfolders",
        callback: { dispatch(.setListMode(.immediate(.cached))) }
      ),
      KBar.Item(
        title: "Match All Filters",
        callback: { dispatch(.setFilterOperator(.and)) }
      ),
      KBar.Item(
        title: "Match Any Filters",
        callback: { dispatch(.setFilterOperator(.or)) }
      ),
    ])
