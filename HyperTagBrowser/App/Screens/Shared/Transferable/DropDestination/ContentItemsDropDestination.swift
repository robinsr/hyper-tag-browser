  ...
  func body(content: Content) -> some View {
    content
      .dropDestination(for: ContentPointers.self) { droppedItems, _ in
        ...
        let droppedContentIds = droppedPointers.ids.filter { $0 != contentItem.id }

        dispatch(
          .applyIndexPatch(
            .location(of: droppedContentIds, with: contentItem.filepath)
          ))

        return true
      } isTargeted: { overTarget in
        if overTarget {
          cursorState.setHoveringTarget(to: contentItem.pointer, with: ContentItem.self)
        } else {
          cursorState.clearHoveringTarget(of: contentItem.pointer, with: ContentItem.self)
        }
      }
  }
}
...
extension View {
   ...
   */
  @ViewBuilder
  func acceptsContentDrops(moveItemTo content: IndexRecord) -> some View {
    if content.conforms(to: .folder) {
      modifier(ContentItemsDropDestination(contentItem: content))
    } else {
      self
    }
  }
