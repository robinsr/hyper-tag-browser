...
/**
 * Supporting extensions of ``NSComparisonPredicate.Operator``
 */
...
extension NSComparisonPredicate.Operator {
  ...
  public func statementValue<T: Comparable>(for value: T, modifiers mods: String = "") -> String {
    switch self {
      case .beginsWith:
        return "*"
          .appendingWord("\(value)", separator: "").quoted
          .appendingWord(mods, separator: "")
      case .endsWith:
        return "\(value)"
          .appendingWord("*", separator: "").quoted
          .appendingWord(mods, separator: "")
      case .contains, .like, .matches:
        return "\(value)"
          .wrap(.ifAbsent("**")).quoted
          .appendingWord(mods, separator: "")
      default:
        return "\(value)".quoted.appendingWord(mods, separator: "")
    }
  }
...
extension NSComparisonPredicate.Operator: @retroactive CustomStringConvertible {
  public var description: String {
    "NSComparisonPredicate.Operator(rawValue: \(self.rawValue), token: \(self.token))"
  }
}
