...
enum DateParsingError: Error {
  ...
  var localizedDescription: String {
    switch self {
    case .invalidDateString(let string):
      return "Invalid date string: \(string)"
    }
  }
}
...
extension Date {
  ...
  func formatted(_ format: DateFormatter) -> String {
    format.string(from: self)
  }
  
  
  static func parseDateString(_ dateString: String) throws -> Date {
    ...
    throw DateParsingError.invalidDateString(dateString)
  }
  ...
  func subtracting(_ component: Calendar.Component, value: Int) -> Date {
    return self.adding(component, value: value * -1)
  }
}
