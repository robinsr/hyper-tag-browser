...
extension JSONEncoder {
  ...
  static func pretty(_ enc: Encodable, omitData data: Bool = true) -> String {
    if let data = try? Self.prettyPrinter(omitData: data).encode(enc) {
      return String(data: data, encoding: .utf8)!
    } else {
      return "{ \"error\": \"failed to encode\" }"
    }
  ...
  static func tryPretty(_ enc: Encodable, omitData data: Bool = true) throws -> String {
    let data = try Self.prettyPrinter(omitData: data).encode(enc)
    
    if let json = String(data: data, encoding: .utf8) {
      return json
    }
    
    throw EncodingError.invalidValue(enc, .init(codingPath: [], debugDescription: "failed to encode to JSON"))
  }
