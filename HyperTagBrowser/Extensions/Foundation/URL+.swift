...
extension URL {
  ...
  /**
   * The equivalent `FilePath` object for this `URL`
   */
  ...
  /**
   * The `FilePath` string (a string by interpreting the path’s content as UTF-8 on Unix and UTF-16 on Windows)
   */
  ...
  /// Indicates whether the item is in iCloud storage.
  var isUbiquitousItem: Bool {
    boolResourceValue(forKey: .isUbiquitousItemKey)
  }
  
  /// The URL path relative to "iCloud Drive
  var ubiquitousRelativePath: String? {
    return fm.componentsToDisplay(forPath: self.string)?
      .drop { $0 != "iCloud Drive" }
      .dropFirst()
      .joined(separator: "/")
  }
